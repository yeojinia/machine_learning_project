...
class Logistic_Regression:
    ...
    def train_with_L1_reg(self, max_iteration, learning_rate, epsilon, lambda_):
        ...
        for idx in range(0, max_iteration):
            ...
            for jdx in range(0, len(self.w_)):
                if (self.w_[jdx] > 0):
                    #self.w_[jdx] -= ((learning_rate) * (gradient_vector[jdx] + lambda_))
                    self.w_[jdx] = max(abs(self.w_[jdx]) - learning_rate*lambda_, 0)
                else:
                    #self.w_[jdx] -= ((learning_rate) * (gradient_vector[jdx] - lambda_))
                    self.w_[jdx] = -max(abs(self.w_[jdx]) - learning_rate*lambda_, 0)
...
def main():
    ...
    for learning_rate in [0.1, 0.01, 0.001, 0.0001, 0.00001]:
        ...
        for hyperparameter in [10 ** 0, 10 ** -1, 10 ** -2, 10 ** -3, 10 ** -4, 10 ** -5]:
            w, cost, iteration = Logistic_Regression(train_data.getX(), train_data.getY()).train_with_L1_reg(
                max_iteration=10000, learning_rate=learning_rate, epsilon=0.5, lambda_=hyperparameter)
        ...
        plt.xticks([0, 1, 2, 3, 4, 5], [10 ** 0, 10 ** -1, 10 ** -2, 10 ** -3, 10 ** -4, 10 ** -5], rotation='horizontal')
        plt.title("(part2) Data Accuracy (Training) with L1 regularization (α= " + str(learning_rate) + " )")
        plt.ylabel('Accuracy')
        plt.xlabel('Lambda(hyperparameter)')
        plt.savefig("LR_" + str(learning_rate) + "part2_training_data_accuracy.png", format='png')
        plt.close()

        fig2, ax2 = plt.subplots()
        plt.plot(validation_accuracy)
        plt.xticks([0, 1, 2, 3, 4, 5], [10 ** 0, 10 ** -1, 10 ** -2, 10 ** -3, 10 ** -4, 10 ** -5], rotation='horizontal')
        plt.title("(part2) Data Accuracy (Validation) with L1 regularization (α= " + str(learning_rate) + " )")
        plt.ylabel('Accuracy')
        plt.xlabel('Lambda(hyperparameter)')
        plt.savefig("LR_" + str(learning_rate) + "part2_validation_data_accuracy.png", format='png')
        plt.close()
        ...
        for lamb, weight, weight_result_ in zip([10 ** 0, 10 ** -1, 10 ** -2, 10 ** -3, 10 ** -4, 10 ** -5], weights, weight_result):
            print(lamb)
            print(Sort_Tuple(weight_result_))
            print("sparsity is " + str(1. - (count_nonzero(weight) / len(weight))), ", the number of feature w_j=0: ",
                  str(len(weight) - count_nonzero(weight)))
